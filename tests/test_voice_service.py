...
@pytest.mark.asyncio
async def test_synthesize_speech_kokoro(voice_service):
    """Test voice synthesis using local Kokoro TTS API."""
    with patch.object(settings, "KOKORO_API_URL", "http://local-kokoro:8880/v1"), \
        ...
        assert call_args[0][1] == "http://local-kokoro:8880/v1/audio/speech"
...
@pytest.mark.asyncio
async def test_synthesize_speech_kokoro_error_handling(voice_service):
    """Test voice synthesis robustly handles API errors without raising exceptions."""
    with patch.object(settings, "KOKORO_API_URL", "http://local-kokoro:8880/v1"), \
         patch("httpx.AsyncClient.stream") as mock_stream:
        ...
        chunks = []
        async for chunk in voice_service.synthesize_speech(text="error"):
            chunks.append(chunk)
...
@pytest.mark.asyncio
async def test_transcribe_audio_webm_fallback_to_wav(voice_service):
    """Test that if Whisper fails on a webm file, it converts to wav and retries."""
    mock_response_fail = MagicMock()
    mock_response_fail.status_code = 400
    mock_response_fail.text = "Bad Content-Type"
    mock_response_fail.raise_for_status.side_effect = httpx.HTTPStatusError(
        "400 Bad Content-Type", request=MagicMock(), response=mock_response_fail
    )
    
    mock_response_success = MagicMock()
    mock_response_success.status_code = 200
    mock_response_success.json.return_value = {"text": "Transcription successful"}
    ...
    with patch("builtins.open", mock_open(read_data=b"audio data")), \
         ...
         patch("pydub.AudioSegment.from_file", return_value=mock_audio) as mock_from_file, \
         patch("httpx.AsyncClient.post") as mock_post, \
         patch("asyncio.sleep") as mock_sleep:
