...
class VoiceService:
    ...
    async def synthesize_speech(
        ...
        """
        Synthesize speech from text using local Kokoro TTS API.
        
        Args:
            text: Text to synthesize
            voice: Voice key (e.g. alloy, af_bella, am_adam)
            model: Model name (e.g. kokoro)
            lang: Language code (e.g. fr, en)
            
        Yields:
            bytes: Audio chunks (MP3/WAV)
        """
        # Map OpenAI voices to Kokoro equivalents
        voice_map = {
            "alloy": "af_bella",
            "echo": "am_adam",
            "fable": "am_michael",
        ...
        # If language is French and voice is default (alloy), use a French voice
        if voice.lower() == "alloy" and lang == "fr":
            kokoro_voice = "ff_siwis"
        else:
            kokoro_voice = voice_map.get(voice.lower(), voice)
            
        # Build URL using Kokoro base URL
        base_url = settings.KOKORO_API_URL or "http://localhost:8880/v1"
        url = f"{base_url}/audio/speech"
        
        logger.info(f"Synthesizing speech via Kokoro TTS API: '{text[:50]}...' (Voice: {kokoro_voice}, Lang: {lang}, URL: {url})")
        
        payload = {
            "model": model if model else "kokoro",
            "input": text,
            "voice": kokoro_voice,
            "response_format": "mp3"
        }
            ...
            async with httpx.AsyncClient(timeout=30.0) as client:
                async with client.stream("POST", url, json=payload) as response:
                    if response.status_code != 200:
                        ...
                        logger.error(
                            f"Kokoro API returned status {response.status_code}: "
                    ...
                    async for chunk in response.aiter_bytes(chunk_size=4096):
                        yield chunk
            logger.info("Kokoro speech synthesis completed successfully")
        except Exception as e:
